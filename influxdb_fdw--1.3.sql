...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'influx_fill_enum') THEN
      CREATE TYPE influx_fill_enum as enum ('linear', 'none', 'null', 'previous');
    END IF;
END$$;
...
--Common functions
SELECT influxdb_create_or_replace_stub('vf', 'log2(float8)', 'float8');
SELECT influxdb_create_or_replace_stub('vf', 'log10(float8)', 'float8');
...
SELECT influxdb_create_or_replace_stub('f', 'influx_time(timestamp with time zone, interval, interval, anyelement)', 'timestamp with time zone');
SELECT influxdb_create_or_replace_stub('f', 'influx_time(timestamp with time zone, interval, anyelement)', 'timestamp with time zone');
SELECT influxdb_create_or_replace_stub('sf', 'influx_fill_option(influx_fill_enum)', 'int');
SELECT influxdb_create_or_replace_stub('sf', 'influx_fill_numeric(float8)', 'float8');
SELECT influxdb_create_or_replace_stub('sf', 'influx_fill_numeric(int)', 'int');
