...
"PGSPIDER_PROJECT_ID"
"PGSPIDER_RPM_ID"
"INFLUXDB_FDW_PROJECT_ID"
"INFLUXDB_FDW_RELEASE_ID"
)
...
for var in "${required_vars[@]}"; do
  if [[ "$var" == "PGSPIDER_RPM_ID" || "$var" == "INFLUXBB_CXX_RPM_ID" ]]; then
    continue
  fi
  ...
  if [[ "$var" == "PGSPIDER_RELEASE_VERSION" || "$var" == "INFLUXDB_CXX_RELEASE_VERSION" ]]; then
    # Check if the value does not conform to the expected version format
  ...
  if [[ "$var" == "PGSPIDER_RELEASE_PACKAGE_VERSION" || "$var" == "INFLUXDB_CXX_RELEASE_VERSION" ]]; then
    # Check if the value does not conform to the expected version format
    if ! [[ "${!var}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      echo "Error: Variable '$var' with value '${!var}' is not in valid version format x.y.z-w."
      missing_vars+=("$var")
