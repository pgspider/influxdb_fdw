...
--Testcase 18:
SELECT 32 + (fields->>'c3')::bigint, (fields->>'c3')::bigint + (fields->>'c3')::bigint from sctbl3 GROUP BY fields->>'c3', fields->>'c5', (fields->>'c2' COLLATE "en_US") ORDER BY (fields->>'c2' COLLATE "en_US");
--Testcase 19:
...
--Testcase 23:
SELECT string_agg(fields->>'c2' COLLATE "en_US", ';' ORDER BY fields->>'c2') FROM sctbl3 GROUP BY fields->>'c2' ORDER BY 1;
--Testcase 24:
...
--Testcase 62:
SELECT upper(fields->>'c2' COLLATE "en_US"), upper(fields->>'c2' COLLATE "en_US"), lower(fields->>'c2' COLLATE "en_US"), lower(fields->>'c2' COLLATE "en_US") FROM sctbl3 ORDER BY 1 ASC,2 DESC,3 ASC,4;
--Testcase 63:
...
--Testcase 74:
SELECT avg((fields->>'c3')::bigint-(fields->>'c3')::bigint-(fields->>'c3')::bigint)-0.567-avg((fields->>'c3')::bigint/3+(fields->>'c3')::bigint)+17.55435, avg((fields->>'c3')::bigint+(fields->>'c3')::bigint/((fields->>'c3')::bigint+45))- -9.5+2*avg((fields->>'c3')::bigint), avg((fields->>'c3')::bigint/((fields->>'c3')::bigint-10.2)*(fields->>'c3')::bigint)+0.567+avg((fields->>'c3')::bigint)*4.5+(fields->>'c3')::bigint, avg((fields->>'c3')::bigint+(fields->>'c3')::bigint/((fields->>'c3')::bigint+5.6))+100-(fields->>'c3')::bigint from sctbl3 WHERE fields->>'c2'<='$' COLLATE "en_US" AND (fields->>'c3')::bigint<>-5 AND (fields->>'c3')::bigint<>10.746  GROUP BY fields->>'c3', fields->>'c3' ORDER BY 1,2,3,4 limit 5;
--Testcase 75:
...
--Testcase 76:
SELECT bit_or((fields->>'c3')::bigint/3*(fields->>'c3')::bigint)-1 + bit_or((fields->>'c3')::bigint/4/((fields->>'c3')::bigint+6)),2* bit_or((fields->>'c3')::bigint-(fields->>'c3')::bigint+(fields->>'c3')::bigint)*1, 5-bit_or((fields->>'c3')::bigint+(fields->>'c3')::bigint-(fields->>'c3')::bigint)-1000000+(fields->>'c3')::bigint  from sctbl3 WHERE fields->>'c2'<='$' COLLATE "en_US" AND (fields->>'c3')::bigint<>-5 AND (fields->>'c3')::bigint<>10.746 GROUP BY fields->>'c3' ORDER BY 1,2,3;
--Testcase 77:
SELECT count(*)-5.6, 2*count(*), 10.2/count(*)+count(*)-(fields->>'c3')::bigint from sctbl3 WHERE (fields->>'c3')::bigint>0.774 OR fields->>'c2' = 'Which started out as a kind' COLLATE "en_US" GROUP BY fields->>'c3' ORDER BY 1, 2, 3;
--Testcase 78:
...
--Testcase 84:
SELECT min(fields->>'c2' COLLATE "en_US"), min((fields->>'c3')::bigint)+(fields->>'c3')::bigint-1, min((fields->>'c3')::bigint+(fields->>'c3')::bigint)-(fields->>'c3')::bigint-3, min((fields->>'c3')::bigint)+min((fields->>'c3')::bigint) from sctbl3 WHERE (fields->>'c4')::double precision<0 GROUP BY fields->>'c3', fields->>'c3' ORDER BY 1,2,3,4;
--Testcase 85:
...
-- Update data
:RECOVER_INIT_MULTILEY_ADD_1TAG_1FIELDS;
--Testcase 202:
...
--Testcase 91:
SELECT * FROM sctbl3 WHERE EXISTS (SELECT (fields->>'c3')::bigint FROM sctbl3 WHERE (fields->>'c3')::bigint != 40.772) ORDER BY tags->>'t1' COLLATE "en_US";
--Testcase 92:
...
-- Select aggregate function  and specific column from original table
--Testcase 97:
SELECT (tags->>'t1' COLLATE "en_US") t1, (fields->>'c1')::bool c1, max(tags->>'t1'), sum((fields->>'c3')::bigint), sum((fields->>'c3')::bigint ORDER BY (fields->>'c3')::bigint) FROM sctbl3 GROUP BY (tags->>'t1' COLLATE "en_US"), (fields->>'c1')::bool, (fields->>'c3')::bigint, fields->>'c2' ORDER BY 1;
--Testcase 98:
...
--Testcase 99:
SELECT stddev((fields->>'c3')::bigint), (fields->>'c1')::bool c1, stddev((fields->>'c3')::bigint ORDER BY (fields->>'c3')::bigint), (tags->>'t1' COLLATE "en_US") t1 FROM sctbl3 GROUP BY time, fields->>'c3', (tags->>'t1' COLLATE "en_US"), fields->>'c1' ORDER BY (tags->>'t1' COLLATE "en_US") ASC,fields->>'c1' DESC;
--Testcase 100:
...
--Testcase 120:
SELECT (fields->>'c1')::bool c1, fields->>'c2' c2, (fields->>'c4')::double precision c4 FROM ( SELECT * FROM sctbl3 WHERE (fields->>'c3')::bigint>-1000 AND (fields->>'c3')::bigint<1000 ) AS tb3 WHERE fields->>'c2'=ANY (ARRAY(SELECT fields->>'c2' FROM sctbl3 WHERE (fields->>'c3')::bigint%2=0)) ORDER BY (tags->>'t1' COLLATE "en_US");
--Testcase 121:
...
--Testcase 136:
SELECT upper(fields->>'c2' COLLATE "en_US"), lower(fields->>'c2' COLLATE "en_US"), lower(tags->>'t1') FROM sctbl3 ORDER BY 1 ASC,2 DESC,3;
--Testcase 137:
...
--Testcase 138:
SELECT (fields->>'c3')::bigint*(random()<=1)::int, (random()<=1)::int*(25-10)+10 FROM sctbl3 ORDER BY (tags->>'t1' COLLATE "en_US") ASC,fields->>'c1' DESC;
--Testcase 139:
SELECT max((fields->>'c3')::bigint), count(*), exists(SELECT * FROM sctbl3 WHERE (fields->>'c3')::bigint>1), exists (SELECT count(*) FROM sctbl3 WHERE (fields->>'c3')::bigint>10.5) FROM sctbl3 GROUP BY fields->>'c3', fields->>'c3' HAVING min((fields->>'c3')::bigint)>min((fields->>'c3')::bigint) ORDER BY 1 ASC,2 DESC,3 ASC,4;
--Testcase 140:
...
-- Update data
:RECOVER_INIT_MULTILEY_ADD_5TAG_20FIELDS;
--Testcase 205:
drop view if exists view_sctbl3 ;
--Testcase 206:
drop foreign table if exists sctbl3;
...
--Testcase 156:
SELECT max(fields->>'c2' COLLATE "en_US"), min((fields->>'c3')::bigint), max((fields->>'c3')::bigint), max((fields->>'c11')::bigint), max((fields->>'c6')::double precision), max((fields->>'c7')::double precision), max((fields->>'c8')::float), max (20) from sctbl3 GROUP BY fields->>'c3' ORDER BY (fields->>'c3')::bigint;
--Testcase 157:
...
--Testcase 172:
SELECT max(time), max(7), max((fields->>'c7')::double precision)+17, max((fields->>'c11')::bigint)+10, max((fields->>'c3')::bigint), max((fields->>'c14')::bigint)/2 FROM ( SELECT * FROM sctbl3 WHERE (fields->>'c3')::bigint>-1000 AND (fields->>'c3')::bigint<1000 ) AS sctbl3 GROUP BY (fields->>'c2' COLLATE "en_US") ORDER BY (fields->>'c2' COLLATE "en_US");
--Testcase 173:
...
--Testcase 175:
SELECT * FROM ( SELECT tags->>'t1' t1, (tags->>'t2')::int t2, (tags->>'t3')::double precision t3, tags->>'t4' t4, tags->>'t5' t5, fields->>'c2' c2, (fields->>'c3')::bigint c3 FROM sctbl3 WHERE (fields->>'c13')::int>-1000 OR (fields->>'c14')::bigint<1000 ) AS tb3 WHERE c3 <-1 AND c2 > 'KissMe' ORDER BY t1 COLLATE "en_US" DESC,2 ASC,3 DESC,4,5 LIMIT 5 OFFSET 0;
--Testcase 176:
...
--Testcase 185:
SELECT bool_and((fields->>'c11')::bigint>0), bool_and((fields->>'c13')::int<0), bool_and((fields->>'c3')::bigint<(fields->>'c3')::bigint), bool_and((fields->>'c7')::double precision>=(fields->>'c8')::float) FROM view_sctbl3 GROUP BY time, (tags->>'t1' COLLATE "en_US"), tags->>'t2',tags->>'t3' ORDER BY (tags->>'t1' COLLATE "en_US") DESC,(tags->>'t2')::float, (tags->>'t3')::double precision;
--Testcase 186:
