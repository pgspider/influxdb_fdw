...
--Testcase 758:
SELECT * from ft1 where (fields->>'c1')::int < 1; -- ERROR
--Testcase 759:
INSERT INTO ft1_nsc (c1) VALUES (1); -- ERROR
--Testcase 760:
DELETE FROM ft1_nsc WHERE c1 = 1; -- ERROR
--Testcase 761:
TRUNCATE ft1; -- ERROR
--Testcase 762:
...
--Testcase 78:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM ft1 t1 WHERE t1.fields->>'c3' IS NULL;        -- NullTest
--Testcase 79:
EXPLAIN (VERBOSE, COSTS OFF) SELECT * FROM ft1 t1 WHERE t1.fields->>'c3' IS NOT NULL;    -- NullTest
--Testcase 80:
...
--Testcase 875:
EXPLAIN (VERBOSE, COSTS OFF)
SELECT (t1.fields->>'c2')::int FROM ft1 t1 WHERE (t1.fields->>'C 1')::int > 960 ORDER BY (t1.fields->>'c2')::int FETCH FIRST 2 ROWS WITH TIES;
--Testcase 876:
SELECT (t1.fields->>'c2')::int FROM ft1 t1 WHERE (t1.fields->>'C 1')::int > 960 ORDER BY (t1.fields->>'c2')::int FETCH FIRST 2 ROWS WITH TIES;
...
--Testcase 879:
EXPLAIN (VERBOSE, COSTS OFF)
	SELECT * FROM ft2 ORDER BY ft2.fields->>'c1', random();
--Testcase 880:
EXPLAIN (VERBOSE, COSTS OFF)
	SELECT * FROM ft2 ORDER BY ft2.fields->>'c1', ft2.fields->>'c3' collate "C";
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT * FROM (
    SELECT 1 AS type,(ft1.fields->>'c1')::int4 as c1 FROM ft1
    UNION ALL
    SELECT 2 AS type,(ft2.fields->>'c1')::int4 as c1 FROM ft2
) a ORDER BY type,c1;
...
-- influxdb_fdw does not support JOIN pushdown
-- SEMI JOIN
--Testcase 175:
EXPLAIN (VERBOSE, COSTS OFF)
...
--Testcase 883:
EXPLAIN (VERBOSE, COSTS OFF)
SELECT t1.c1, t2.c1 FROM (SELECT (fields->>'C 1')::int c1, (fields->>'c2')::int c2, tags->>'c3' c3, fields->>'c4' c4, fields->>'c5' c5, fields->>'c6' c6, fields->>'c7' c7, fields->>'c8' c8 FROM ft1) t1 JOIN (SELECT (fields->>'C 1')::int c1, (fields->>'c2')::int c2, tags->>'c3' c3, fields->>'c4' c4, fields->>'c5' c5, fields->>'c6' c6, fields->>'c7' c7, fields->>'c8' c8 FROM ft2) t2 ON (t1.c1 = t2.c1 AND CURRENT_USER = SESSION_USER) ORDER BY t1.c3, t1.c1 OFFSET 100 LIMIT 10;
...
--Testcase 868:
EXPLAIN (VERBOSE, COSTS OFF)
SELECT * FROM local_tbl LEFT JOIN (SELECT ft1.* FROM ft1 INNER JOIN ft2 ON ((ft1.fields->>'C 1')::int = (ft2.fields->>'C 1')::int AND (ft1.fields->>'C 1')::int < 100 AND ((ft1.fields->>'C 1')::int - influxdb_fdw_abs((ft2.fields->>'C 1')::int)) = 0)) ss ON (local_tbl.fields->>'c3' = ss.tags->>'c3') ORDER BY local_tbl.fields->>'c1' FOR UPDATE OF local_tbl;
...
--Testcase 895:
EXPLAIN (verbose, costs off)
SELECT (ft2.fields->>'C 1')::int4 as c1, (ft2.fields->>'c2')::int4 as c2, (ft2.tags->>'c3') as c3, (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
       (ft4.fields->>'C 1')::int4 as c1, (ft4.fields->>'c2')::int4 as c2, (ft4.tags->>'c3') as c3
  FROM ft2 INNER JOIN ft4 ON (ft2.fields->>'c2')::int4 = (ft4.fields->>'c1')::int4
  WHERE (ft2.fields->>'C 1')::int4 > 900
  AND EXISTS (SELECT 1 FROM ft5 WHERE (ft4.fields->>'c1')::int4 = (ft5.fields->>'c1')::int4)
  ORDER BY (ft2.fields->>'C 1')::int4;
--Testcase 896:
SELECT (ft2.fields->>'C 1')::int4 as c1, (ft2.fields->>'c2')::int4 as c2, (ft2.tags->>'c3') as c3, (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
       (ft4.fields->>'c1')::int4 as c1, (ft4.fields->>'c2')::int4 as c2, (ft4.tags->>'c3') as c3
  FROM ft2 INNER JOIN ft4 ON (ft2.fields->>'c2')::int4 = (ft4.fields->>'c1')::int4
  WHERE (ft2.fields->>'C 1')::int4 > 900
  AND EXISTS (SELECT 1 FROM ft5 WHERE (ft4.fields->>'c1')::int4 = (ft5.fields->>'c1')::int4)
  ORDER BY (ft2.fields->>'C 1')::int4;
...
--Testcase 897:
EXPLAIN (verbose, costs off)
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
       ft4.*
  FROM ft2 INNER JOIN
  (SELECT (ft4.fields->>'c1')::int4 as c1, (ft4.fields->>'c2') as c2, (ft4.tags->>'c3') as c3 FROM ft4 WHERE
  EXISTS (SELECT 1 FROM ft5 WHERE (ft4.fields->>'c1')::int4 = (ft5.fields->>'c1')::int4)) ft4
  ON (ft2.fields->>'c2')::int4 = ft4.c1
  WHERE (ft2.fields->>'C 1')::int4 > 900
  ORDER BY (ft2.fields->>'C 1')::int4;
--Testcase 898:
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
...
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
       ft4.*
  FROM ft2 LEFT JOIN
  (SELECT (ft4.fields->>'c1')::int4 as c1, (ft4.fields->>'c2'), (ft4.tags->>'c3') FROM ft4 WHERE
  EXISTS (SELECT 1 FROM ft5 WHERE (ft4.fields->>'c1')::int4 = (ft5.fields->>'c1')::int4)) ft4
  ON (ft2.fields->>'c2')::int4 = ft4.c1
  WHERE (ft2.fields->>'C 1')::int4 > 900
  ORDER BY (ft2.fields->>'C 1')::int4 LIMIT 10;
--Testcase 900:
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
       ft4.*
  FROM ft2 LEFT JOIN
...
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
  ...
  EXISTS (SELECT 1 FROM ft5 WHERE (ft4.fields->>'c1')::int4 = (ft5.fields->>'c1')::int4)) ft4
  ON (ft2.fields->>'c2')::int4 = ft4.c1
  INNER JOIN (SELECT (ft5.fields->>'c1')::int4 as c1, (ft5.fields->>'c2'), (ft5.tags->>'c3') FROM ft5 WHERE
  EXISTS (SELECT 1 FROM ft4 WHERE (ft4.fields->>'c1')::int4 = (ft5.fields->>'c1')::int4)) ft5
  ON (ft2.fields->>'c2')::int4 <= ft5.c1
  WHERE (ft2.fields->>'C 1')::int4 > 900
  ORDER BY (ft2.fields->>'C 1')::int4 LIMIT 10;
--Testcase 902:
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
...
--Testcase 903:
EXPLAIN (verbose, costs off)
SELECT (ft2.fields->>'C 1') as c1, (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8') FROM ft2 WHERE
  (ft2.fields->>'C 1')::int4 = ANY (
	SELECT (ft2.fields->>'C 1')::int4 FROM ft2 WHERE
	  EXISTS (SELECT 1 FROM ft4 WHERE (ft4.fields->>'c2')::int4 = (ft2.fields->>'c2')::int4))
  AND (ft2.fields->>'C 1')::int4 > 900
  ORDER BY (ft2.fields->>'C 1')::int4 LIMIT 10;
--Testcase 904:
SELECT (ft2.fields->>'C 1') as c1, (ft2.fields->>'c2'), (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8') FROM ft2 WHERE
  (ft2.fields->>'C 1')::int4 = ANY (
...
--Testcase 905:
EXPLAIN (verbose, costs off)
SELECT (ftupper.fields->>'C 1'), (ftupper.fields->>'c2'), (ftupper.tags->>'c3'), (ftupper.fields->>'c6'), (ftupper.fields->>'c7'), (ftupper.fields->>'c8') FROM ft2 ftupper WHERE
   EXISTS (
	SELECT (ft2.fields->>'C 1')::int4 as c1 FROM ft2 WHERE
	  EXISTS (SELECT 1 FROM ft4 WHERE (ft4.fields->>'c2')::int4 = (ft2.fields->>'c2')::int4) AND (ft2.fields->>'C 1')::int4 = (ftupper.fields->>'C 1')::int4 )
  AND (ftupper.fields->>'C 1')::int4 > 900
  ORDER BY (ftupper.fields->>'C 1')::int4 LIMIT 10;
--Testcase 906:
SELECT (ftupper.fields->>'C 1'), (ftupper.fields->>'c2'), (ftupper.tags->>'c3'), (ftupper.fields->>'c6'), (ftupper.fields->>'c7'), (ftupper.fields->>'c8') FROM ft2 ftupper WHERE
   EXISTS (
	SELECT (ft2.fields->>'C 1')::int4 as c1 FROM ft2 WHERE
...
--Testcase 907:
EXPLAIN (verbose, costs off)
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2')::int4 as c2, (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
       ft4.*
  FROM ft2 INNER JOIN
	(SELECT (ft4.fields->>'c1')::int4 as c1, (ft4.fields->>'c2'), (ft4.tags->>'c3') FROM ft4 WHERE EXISTS (
		SELECT 1 FROM ft2 WHERE (ft2.fields->>'c2')::int4 = (ft4.fields->>'c2')::int4)) ft4
	ON (ft2.fields->>'c2')::int4 = ft4.c1
	INNER JOIN
	(SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2')::int4 as c2, (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8')
    FROM ft2 WHERE EXISTS (
		SELECT 1 FROM ft4 WHERE (ft2.fields->>'c2')::int4 = (ft4.fields->>'c2')::int4)) ft21
	ON (ft2.fields->>'c2')::int4 = ft21.c2
	WHERE (ft2.fields->>'C 1')::int4 > 900
	ORDER BY (ft2.fields->>'C 1')::int4 LIMIT 10;
--Testcase 908:
SELECT (ft2.fields->>'C 1'), (ft2.fields->>'c2')::int4 as c2, (ft2.tags->>'c3'), (ft2.fields->>'c6'), (ft2.fields->>'c7'), (ft2.fields->>'c8'),
...
--Testcase 909:
EXPLAIN (verbose, costs off)
SELECT (ft1.fields->>'C 1')::int4 as c1 FROM ft1 JOIN ft2 on (ft1.fields->>'C 1')::int4 = (ft2.fields->>'C 1')::int4 WHERE
	(ft1.fields->>'C 1')::int4 IN (
		SELECT (ft2.fields->>'C 1')::int4 FROM ft2 JOIN ft4 ON (ft2.fields->>'C 1')::int4 = (ft4.fields->>'C 1')::int4)
	ORDER BY (ft1.fields->>'C 1')::int4 LIMIT 5;
...
-- InfluxDB FDW does not support UPDATE command.
...
--Testcase 913:
CREATE FOREIGN TABLE child_foreign_nsc (b text, c numeric, a int)
  SERVER influxdb_svr OPTIONS (table 'child_local');
--Testcase 914:
CREATE FOREIGN TABLE child_foreign (fields jsonb OPTIONS(fields 'true')) SERVER influxdb_svr OPTIONS (table 'child_local', schemaless 'true');
ALTER TABLE sub_parent ATTACH PARTITION child_foreign_nsc FOR VALUES FROM (1) TO (10);
--Testcase 915:
...
--Testcase 923:
DROP FOREIGN TABLE child_foreign_nsc;
--Testcase 924:
...
--Testcase 936:
drop foreign table tab_batch_sharded_p1_remote;
--Testcase 937:
drop foreign table tab_batch_sharded_p1;
--Testcase 773:
alter server influxdb_svr options (drop batch_size);
...
--Testcase 541:
CREATE TRIGGER trig_stmt_before BEFORE DELETE OR INSERT OR UPDATE OR TRUNCATE ON rem1_nsc
	FOR EACH STATEMENT EXECUTE PROCEDURE trigger_func();
--Testcase 542:
CREATE TRIGGER trig_stmt_after AFTER DELETE OR INSERT OR UPDATE OR TRUNCATE ON rem1_nsc
	FOR EACH STATEMENT EXECUTE PROCEDURE trigger_func();
...
--Testcase 959:
CREATE FOREIGN TABLE ftable_nsc (a text, b int)
	SERVER influxdb_svr
	OPTIONS (table 'batch_table', batch_size '2');
--Testcase 960:
CREATE FOREIGN TABLE ftable (fields jsonb OPTIONS(fields 'true')) 
	SERVER influxdb_svr OPTIONS ( table 'batch_table', batch_size '2', schemaless 'true' );
--Testcase 961:
...
INSERT INTO ftable_nsc SELECT * FROM t;
...
-- Test from query_cancel.sql
-- InfluxDB FDW does not support asynchronous processing
-- SELECT version() ~ 'cygwin' AS skip_test \gset
-- \if :skip_test
-- \quit
-- \endif

-- -- Let's test canceling a remote query.
-- -- influxdb_fdw does not support JOIN pushdown. Therefore, plan of this test case does not pushdown JOIN.
-- EXPLAIN (VERBOSE, COSTS OFF)
