...
--Testcase 671:
explain (costs off)
select (t1.fields->>'f1')::int4 as f1
from int4_tbl t1, int4_tbl t2
  left join int4_tbl t3 on (t3.fields->>'f1')::int4 > 0
  left join int4_tbl t4 on (t3.fields->>'f1')::int4 > 1
where (t4.fields->>'f1')::int4 is null;
...
from int4_tbl t1 left join int4_tbl t2 on true
  left join int4_tbl t3 on (t2.fields->>'f1')::int4 > 0
  left join int4_tbl t4 on (t3.fields->>'f1')::int4 > 0;

--Testcase 674:
explain (costs off)
select * from onek t1
  left join onek t2 on (t1.fields->>'unique1')::int4 = (t2.fields->>'unique1')::int4
  left join onek t3 on (t2.fields->>'unique1')::int4 != (t3.fields->>'unique1')::int4
  left join onek t4 on (t3.fields->>'unique1')::int4 = (t4.fields->>'unique1')::int4;

--Testcase 675:
explain (costs off)
select * from int4_tbl t1
  left join (select now() from int4_tbl t2
             left join int4_tbl t3 on (t2.fields->>'f1')::int4 = (t3.fields->>'f1')::int4
             left join int4_tbl t4 on (t3.fields->>'f1')::int4 = (t4.fields->>'f1')::int4) s on true
...
select * from int4_tbl t1
  ...
  left join int4_tbl t4 on (t2.fields->>'f1')::int4 = (t3.fields->>'f1')::int4;
...
select * from int4_tbl t1
  left join int4_tbl t2 on true
  left join int4_tbl t3 on (t2.fields->>'f1')::int4 = (t3.fields->>'f1')::int4
  left join int4_tbl t4 on (t3.fields->>'f1')::int4 != (t4.fields->>'f1')::int4;

--Testcase 678:
explain (costs off)
select * from int4_tbl t1
  left join (int4_tbl t2 left join int4_tbl t3 on (t2.fields->>'f1')::int4 > 0) on (t2.fields->>'f1')::int4 > 1
  left join int4_tbl t4 on (t2.fields->>'f1')::int4 > 2 and (t3.fields->>'f1')::int4 > 3
where (t1.fields->>'f1')::int4 = coalesce((t2.fields->>'f1')::int4, 1);

--Testcase 679:
explain (costs off)
select * from int4_tbl t1
  left join ((select (t2.fields->>'f1')::int4 as f1 from int4_tbl t2
                left join int4_tbl t3 on (t2.fields->>'f1')::int4 > 0
                where (t3.fields->>'f1')::int4 is null) s
             left join tenk1 t4 on s.f1 > 1)
    on s.f1 = (t1.fields->>'f1')::int4;
...
select * from int4_tbl t1
  left join ((select (t2.fields->>'f1')::int4 as f1 from int4_tbl t2
                left join int4_tbl t3 on (t2.fields->>'f1')::int4 > 0
                where (t2.fields->>'f1')::int4 <> coalesce((t3.fields->>'f1')::int4, -1)) s
             left join tenk1 t4 on s.f1 > 1)
    on s.f1 = (t1.fields->>'f1')::int4;
...
select * from onek t1
    left join onek t2 on (t1.fields->>'unique1')::int4 = (t2.fields->>'unique1')::int4
    left join onek t3 on (t2.fields->>'unique1')::int4 = (t3.fields->>'unique1')::int4
    left join onek t4 on (t3.fields->>'unique1')::int4 = (t4.fields->>'unique1')::int4 and (t2.fields->>'unique2')::int4 = (t4.fields->>'unique2')::int4;

--Testcase 682:
explain (costs off)
select * from int8_tbl t1 left join
    (int8_tbl t2 left join int8_tbl t3 full join int8_tbl t4 on false on false)
    left join int8_tbl t5 on (t2.fields->>'q1')::int4 = (t5.fields->>'q1')::int4
on (t2.fields->>'q2')::int4 = 123;
...
select * from int8_tbl t1
    ...
    left join lateral
      (select * from int8_tbl t3 where (t3.fields->>'q1')::int4 = (t2.fields->>'q1')::int4 offset 0) s
      on (t2.fields->>'q1')::int4 = 1;
...
select * from int8_tbl t1
    left join int8_tbl t2 on true
    left join lateral
      (select * from generate_series((t2.fields->>'q1')::int4, 100)) s
      on (t2.fields->>'q1')::int4 = 1;
...
select * from int8_tbl t1
    left join int8_tbl t2 on true
    left join lateral
      (select (t2.fields->>'q1')::int4 from int8_tbl t3) s
      on (t2.fields->>'q1')::int4 = 1;
...
select * from onek t1
    ...
    left join lateral
      (select * from onek t3 where (t3.fields->>'two')::int4 = (t2.fields->>'two')::int4 offset 0) s
      on (t2.fields->>'unique1')::int4 = 1;
...
--Testcase 687:
explain (costs off)
select (x.fields->>'thousand')::int4 as thousand, (x.fields->>'twothousand')::int4 as twothousand, count(*)
from tenk1 x inner join tenk1 y on (x.fields->>'thousand')::int4 = (y.fields->>'thousand')::int4
group by (x.fields->>'thousand'), (x.fields->>'twothousand')
order by (x.fields->>'thousand') desc, (x.fields->>'twothousand');
...
--
--Testcase 757:
create foreign table tbl_ra(fields jsonb OPTIONS (fields 'true')) server influxdb_svr OPTIONS(schemaless 'true');
--Testcase 758:
create foreign table tbl_ra_nsc(a int, b int) server influxdb_svr OPTIONS (table 'tbl_ra');
--Testcase 759:
insert into tbl_ra_nsc select i, i%100 from generate_series(1,1000)i;
...
-- Due to cannot create index on a foreign table, plan is not merge right anti join
--Testcase 762:
explain (costs off)
select * from tbl_ra t1
where not exists (select 1 from tbl_ra t2 where (t2.fields->>'b')::int = (t1.fields->>'a')::int) and (t1.fields->>'b')::int < 2;
...
--Testcase 688:
EXPLAIN (COSTS OFF)
SELECT (a.fields->>'f1')::int4 as f1
FROM tt4 a
LEFT JOIN (
        SELECT (b.fields->>'f1')::int4 as f1
        FROM tt3 b LEFT JOIN tt3 c ON ((b.fields->>'f1')::int4 = (c.fields->>'f1')::int4)
        WHERE COALESCE((c.fields->>'f1')::int4, 0) = 0
) AS d ON ((a.fields->>'f1')::int4 = d.f1)
...
where (a.fields->>'unique1')::int4 in (select (b.fields->>'unique2')::int4 from tenk1 b);

-- sadly, this is not an antijoin
--Testcase 691:
explain (costs off)
select a.* from tenk1 a
where (a.fields->>'unique1')::int4 not in (select (b.fields->>'unique2')::int4 from tenk1 b);

--Testcase 692:
explain (costs off)
select a.* from tenk1 a
where exists (select 1 from tenk1 b where (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4);

--Testcase 693:
explain (costs off)
select a.* from tenk1 a
where not exists (select 1 from tenk1 b where (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4);

--Testcase 694:
explain (costs off)
select a.* from tenk1 a left join tenk1 b on (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4
where (b.fields->>'unique2')::int4 is null;
...
$$ select (t.fields->>'unique2')::int4; $$ language sql immutable;

--Testcase 700:
explain (verbose, costs off)
select t2.fields->>'unique1', f_field_select(t2) from tenk1 t1
    left join onek t2 on (t1.fields->>'unique1')::int4 = (t2.fields->>'unique1')::int4
...
--Testcase 702:
explain (costs off)
select * from int8_tbl t1 left join int8_tbl t2 on (t1.fields->>'q2')::int8 = (t2.fields->>'q1')::int8,
  lateral (select * from int8_tbl t3 where (t2.fields->>'q1')::int8 = (t2.fields->>'q2')::int8) ss;
...
( (select (fields->>'f1')::int4 f1, 123 as x from int4_tbl i1) ss1
  left join
  (select (fields->>'q1') q1, (fields->>'q2') q2, (fields->>'q2')::int4 as x from int8_tbl i2) ss2
  using (x)
) ss0
on ((i0.fields->>'f1')::int4 = ss0.f1)
order by (i0.fields->>'f1')::int4, x;
...
select 1 from
  int4_tbl i4
  left join int8_tbl i8 on (i4.fields->>'f1')::int4 is not null
  left join (select 1 as a) ss1 on null
  join int4_tbl i42 on ss1.a is null or (i8.fields->>'q1')::int8 <> (i8.fields->>'q2')::int8
  right join (select 2 as b) ss2
  on ss2.b < (i4.fields->>'f1')::int4;
...
select 1 from
  int4_tbl as i4
  inner join
    ((select 42 as n from int4_tbl x1 left join int8_tbl x2 on (x1.fields->>'f1')::int4 = (x2.fields->>'q1')::int8) as ss1
     right join (select 1 as z) as ss2 on true)
  on false,
  lateral (select (i4.fields->>'f1')::int4 f1, ss1.n from int8_tbl as i8 limit 1) as ss3;
...
--Testcase 710:
CREATE FOREIGN TABLE temp_t (fields jsonb OPTIONS (fields 'true')) SERVER influxdb_svr OPTIONS(schemaless 'true');

--Testcase 711:
explain (costs off)
select 1 from (select (temp_t.fields->>'a')::int4 a FROM temp_t) t1
  join lateral (select t1.a from (select 1) foo offset 0) as s1 on true
  join
    (select 1 from (select (temp_t.fields->>'a')::int4 a FROM temp_t) t2
       inner join ((select (temp_t.fields->>'a')::int4 a FROM temp_t) t3
                   left join ((select (temp_t.fields->>'a')::int4 a FROM temp_t) t4 left join (select (temp_t.fields->>'a')::int4 a FROM temp_t) t5 on t4.a = 1)
                   on t3.a = t4.a)
       on false
...
--Testcase 712:
explain (costs off)
select (a.fields->>'unique1') unique1, (b.fields->>'unique2') unique2
  from onek a left join onek b on (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4
  where ((b.fields->>'unique2')::int4, random() > 0) = any (select (fields->>'q1')::int8, random() > 0 from int8_tbl c where (c.fields->>'q1')::int8 < (b.fields->>'unique1')::int4);
...
explain (costs off)
select a.fields->>'unique1' unique1, b.fields->>'unique2' unique2
  from onek a full join onek b on (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4
  where (a.fields->>'unique1')::int4 = 42;
...
select a.fields->>'unique1' unique1, b.fields->>'unique2' unique2
  from onek a full join onek b on (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4
  where (b.fields->>'unique2')::int4 = 43;
...
--Testcase 718:
explain (costs off)
select a.fields->>'unique1', b.fields->>'unique2'
  from onek a full join onek b on (a.fields->>'unique1')::int4 = (b.fields->>'unique2')::int4
  where (a.fields->>'unique1')::int4 = 42 and (b.fields->>'unique2')::int4 = 42;
...
explain (costs off)
select * from
  (select * from int8_tbl i81 join (values(123,2)) v(v1,v2) on (i81.fields->>'q2')::int8=v1) ss1
full join
  (select * from (values(456,2)) w(v1,v2) join int8_tbl i82 on (i82.fields->>'q2')::int8=v1) ss2
on true;
...
--Testcase 722:
explain (costs off)
select a1.fields->>'id' id from
  (a a1 left join a a2 on true)
  left join
  (a a3 left join a a4 on (a3.fields->>'id')::int = (a4.fields->>'id')::int)
  on (a2.fields->>'id')::int = (a3.fields->>'id')::int;

--Testcase 723:
explain (costs off)
select a1.fields->>'id' id from
  (a a1 left join a a2 on (a1.fields->>'id')::int = (a2.fields->>'id')::int)
  left join
  (a a3 left join a a4 on (a3.fields->>'id')::int = (a4.fields->>'id')::int)
  on (a2.fields->>'id')::int = (a3.fields->>'id')::int;
...
select 1 from a t1
   ...
   inner join a t3 on true
    left join a t4 on (t2.fields->>'id')::int = (t4.fields->>'id')::int and (t2.fields->>'id')::int = (t3.fields->>'id')::int;
...
from int4_tbl as t1
  left join (int4_tbl as t2
             ...
             left join (int4_tbl as t4
                        right join int8_tbl as t5 on null)
               on (t2.fields->>'f1')::int = (t4.fields->>'f1')::int
             left join ((select t6.fields->>'f1' f1 from int4_tbl as t6) as ss1
                        inner join int8_tbl as t7 on null)
               on (t5.fields->>'q1')::int = (t7.fields->>'q2')::int)
    on false;
...
explain (costs off)
select ss1.x
from (select (i4.fields->>'f1')::int4/2 as x from int4_tbl i4 left join a on (a.fields->>'id')::int4 = (i4.fields->>'f1')::int4) ss1
...
select *
from int8_tbl t1
  left join (int8_tbl t2 left join onek t3 on (t2.fields->>'q1')::int8 > (t3.fields->>'unique1')::int8)
    on (t1.fields->>'q2')::int8 = (t2.fields->>'q2')::int8
  left join onek t4
    on (t2.fields->>'q2')::int8 < (t3.fields->>'unique2')::int8;
...
select * from int8_tbl t1 left join
  (int8_tbl t2 inner join int8_tbl t3 on false
   left join int8_tbl t4 on (t2.fields->>'q2')::int8 = (t4.fields->>'q2')::int8)
on (t1.fields->>'q1')::int8 = (t2.fields->>'q1')::int8;

-- deduce constant-false from an EquivalenceClass
--Testcase 730:
explain (costs off)
select * from int8_tbl t1 left join
  (int8_tbl t2 inner join int8_tbl t3 on ((t2.fields->>'q1')::int8-(t3.fields->>'q2')::int8) = 0 and ((t2.fields->>'q1')::int8-(t3.fields->>'q2')::int8) = 1
   left join int8_tbl t4 on (t2.fields->>'q2')::int8 = (t4.fields->>'q2')::int8)
on (t1.fields->>'q1')::int8 = (t2.fields->>'q1')::int8;
...
select exists(
  select * from int8_tbl t1 left join
    (int8_tbl t2 inner join int8_tbl t3 on (x0.fields->>'f1')::int4 = 1
     left join int8_tbl t4 on (t2.fields->>'q2')::int8 = (t4.fields->>'q2')::int8)
  on (t1.fields->>'q1')::int8 = (t2.fields->>'q1')::int8
) from int4_tbl x0;
...
explain (costs off)
select 1 from a t1
  left join (a t2 left join a t3 on (t2.fields->>'id')::int = 1) on (t2.fields->>'id')::int = 1;
...
--Testcase 733:
explain (costs off)
select c.fields->>'id', ss.a from c
  left join (select d.fields->>'a' a from onerow, d left join b on (d.fields->>'a')::int = (b.fields->>'id')::int) ss
  on (c.fields->>'id')::int = ss.a::int;
...
--Testcase 736:
explain (costs off)
select a.* from a left join parted_b pb on (a.fields->>'b_id')::int = pb.id;
...
EXPLAIN (COSTS OFF)
SELECT q2 FROM
  (SELECT (int8_tbl.fields->>'q1')::int8 q1, (int8_tbl.fields->>'q2')::int8 q2, (innertab.fields->>'id')::int id, (innertab.fields->>'dat1')::int dat1
   FROM int8_tbl LEFT JOIN innertab ON (int8_tbl.fields->>'q2')::int8 = (innertab.fields->>'id')::int) ss
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT q2 FROM
  (SELECT (int8_tbl.fields->>'q2')::int8 q2, 'constant'::text AS x
   FROM int8_tbl LEFT JOIN innertab ON (int8_tbl.fields->>'q2')::int8 = (innertab.fields->>'id')::int) ss
...
--Testcase 739:
EXPLAIN (COSTS OFF)
SELECT (int4_tbl.fields->>'f1'), x
FROM int4_tbl
     JOIN ((SELECT 42 AS x FROM int8_tbl LEFT JOIN innertab ON (int8_tbl.fields->>'q1')::int8 = (innertab.fields->>'id')::int) AS ss1
           RIGHT JOIN tenk1 ON NULL)
        ON (tenk1.fields->>'unique1')::int = ss1.x OR (tenk1.fields->>'unique2')::int = ss1.x;
...
--Testcase 740:
create foreign table t (fields jsonb OPTIONS (fields 'true')) server influxdb_svr OPTIONS (table 't1_eq_class', schemaless 'true');
--Testcase 741:
create foreign table t_tmp (a int) server influxdb_svr OPTIONS (table 't1_eq_class');
--Testcase 742:
...
explain (costs off)
select 1
from (select (fields->>'a')::int as a from t) t1
  left join (select 2 as c
             from (select (fields->>'a')::int as a from t) t2 left join (select (fields->>'a')::int as a from t) t3 on t2.a = t3.a) s
    on true
where t1.a = s.c;
...
--Testcase 745:
create foreign table t (fields jsonb OPTIONS (fields 'true')) server influxdb_svr OPTIONS (table 't2_eq_class', schemaless 'true');
--Testcase 746:
create foreign table t_tmp (a int, b int) server influxdb_svr OPTIONS (table 't2_eq_class');
--Testcase 747:
insert into t_tmp values (1,1), (2,2);

--Testcase 748:
explain (costs off)
select 1
from (select (fields->>'a')::int as a, (fields->>'b')::int as b from t) t1
  left join (select t2.a, 1 as c
             from (select (fields->>'a')::int as a, (fields->>'b')::int as b from t) t2 left join (select (fields->>'a')::int as a, (fields->>'b')::int as b from t) t3 on t2.a = t3.a) s
  on true
  left join (select (fields->>'a')::int as a, (fields->>'b')::int as b from t) t4 on true
where s.a < s.c;
...
--Testcase 751:
select (ffields->>'ctid')::int4 ctid from
  tenk1 t1 join tenk2 t2 on (t1.fields->>'two')::int4 = (t2.fields->>'two')::int4; -- error, need qualification
...
explain (verbose, costs off)
select 1 from
  (select * from int8_tbl where (int8_tbl.fields->>'q1')::int8 <> (select 42) offset 0) ss
where false;
...
explain (costs off)
select * from int4_tbl t1,
  lateral (select * from int4_tbl t2 inner join int4_tbl t3 on (t1.fields->>'f1')::int = 1
...
lateral (select * from (select (fields->>'q1')::int8 q1, (fields->>'q2')::int8 q2 from INT8_TBL) t1,
         lateral (select * from
                    (select * from (select (fields->>'q1')::int8 q1, (fields->>'q2')::int8 q2 from INT8_TBL) t2
                     where (q1, random() > 0) = any (select (fields->>'q2')::int8, random() > 0 from int8_tbl t3
...
--Testcase 756:
explain (verbose, costs off)
select * from (select (fields->>'id2')::int id2 from j1) j1
inner join (select (fields->>'id2')::int id2 from j2) j2 on (j1.id2)::int = (j2.id2)::int;
...
-- This test not suitable for influxdb_fdw
--   1. influxdb_fdw not support JOIN pushdown
