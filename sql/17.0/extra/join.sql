...
--
--Testcase 124:
CREATE FOREIGN TABLE b_star(class char, aa int4, bb text, a int4) SERVER influxdb_svr;
...
--
--Testcase 757:
create foreign table tbl_ra(a int,  b int) server influxdb_svr;
--Testcase 758:
...
--Testcase 677:
CREATE FOREIGN TABLE temp_t (a int) SERVER influxdb_svr;

--Testcase 678:
explain (costs off)
select 1 from temp_t t1
  join lateral (select t1.a from (select 1) foo offset 0) as s1 on true
  join
    (select 1 from temp_t t2
       inner join (temp_t t3
                   left join (temp_t t4 left join temp_t t5 on t4.a = 1)
                   on t3.a = t4.a)
       on false
...
--Testcase 680:
create foreign table t (a int) server influxdb_svr OPTIONS (table 't1_eq_class');
--Testcase 681:
...
--Testcase 685:
create foreign table t (a int, b int) server influxdb_svr OPTIONS (table 't2_eq_class');
--Testcase 686:
