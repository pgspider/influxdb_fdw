...
--Testcase 514:
CREATE FOREIGN TABLE v1 (v int4) SERVER influxdb_svr OPTIONS (table 'v1');

--Testcase 515:
INSERT INTO v1 (v) VALUES (1), (2), (3);
--Testcase 516:
...
-- NULL datatype is not supported in influxdb. Expectation is error.
--Testcase 521:
INSERT INTO v1 (v) VALUES (NULL), (1), (2);
--Testcase 522:
SELECT any_value(v) FROM v1;
--Testcase 523:
DELETE FROM v1;
...
--Testcase 524:
CREATE FOREIGN TABLE v2 (v text[]) SERVER influxdb_svr OPTIONS (table 'v2');

-- Cannot binding text array. Expectation is error.
--Testcase 525:
INSERT INTO v2 (v) VALUES (array['hello', 'world']);
--Testcase 526:
...
--Testcase 215:
create foreign table agg_t1(f1 int, f2 int) server influxdb_svr;
--Testcase 216:
create foreign table agg_t2(f1 bigint, f2 oid) server influxdb_svr;
...
agg_t1 x(x0,x1) left join (agg_t1 left join agg_t2 using(f1)) on (x0 = 0)
group by f1;

-- same, for a RelabelType coercion
--Testcase 530:
select f2, count(*) from
agg_t1 x(x0,x1) left join (agg_t1 left join agg_t2 using(f2)) on (x0 = 0)
group by f2;
...
--Testcase 556:
create foreign table agg_t6 (c1 text, id int) server influxdb_svr options (table 'agg_t6');
--Testcase 557:
create foreign table agg_t7 (id int) server influxdb_svr options (table 'agg_t7');
--Testcase 558:
insert into agg_t6 values ('a', 1);
--Testcase 559:
insert into agg_t7 values (1);
--Testcase 560:
  select
...
--Testcase 562:
CREATE FOREIGN TABLE btg (x int, y int, z text, w int) SERVER influxdb_svr OPTIONS (table 'btg_groupby'); 
--Testcase 563:
INSERT INTO btg
  SELECT
    i % 10 AS x,
...
-- pathkeys
--Testcase 571:
CREATE FOREIGN TABLE group_agg_pk (x int, y int, z int, w int, f int) SERVER influxdb_svr OPTIONS (table 'group_agg_pk'); 
--Testcase 572:
