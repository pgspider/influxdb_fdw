...
def make_check_test(String target, String version) {
    ...
    catchError() {
        sh """
            ...
            docker exec -u postgres -w /home/postgres/${target}${version}/contrib/influxdb_fdw postgresserver_multi_for_influxdb_existed_test /bin/bash -c 'export PATH=$PATH:/usr/local/go/bin && go get github.com/influxdata/influxdb1-client/v2 && export LANGUAGE="en_US.UTF-8" && export LANG="en_US.UTF-8" && export LC_ALL="en_US.UTF-8" && make ${prefix} && make check ${prefix} | tee make_check.out'
